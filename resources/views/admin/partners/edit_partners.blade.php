        ...
        <div class="col-12 grid-margin stretch-card">
          ...
          <div class="card-body">
            <div class="pb-3 d-flex align-items-center justify-content-between">
              ...
              <form action="{{route('partners.update',$partner)}}" method="POST" class="forms-sample" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="name">Name :</label>
                    <input type="text" name="name" class="form-control text-white" id="name" placeholder="name" value="{{$partner->name}}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="url" class="form-control text-white" id="url" placeholder="url" value="{{$partner->url}}">
                    @error('url')
                    <p class="text-danger">{{$message}}</p>
                    @enderror
                  </div>
